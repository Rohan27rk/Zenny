...
export function Login({ onToggleAuth }: LoginProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (error) { setError(error.message || 'Failed to sign in'); setLoading(false); }
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden"
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="rounded-3xl border border-white/10 p-8 backdrop-blur-xl"
          style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.08), rgba(255,255,255,0.03))', boxShadow: '0 25px 60px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.1)' }}>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  style={{ background: 'rgba(0,0,0,0.3)', borderColor: 'rgba(255,255,255,0.1)', boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.3)' }}
                  onFocus={e => { e.target.style.borderColor = 'rgba(59,130,246,0.6)'; e.target.style.boxShadow = 'inset 0 2px 4px rgba(0,0,0,0.3), 0 0 0 3px rgba(59,130,246,0.15)'; }}
                  onBlur={e => { e.target.style.borderColor = 'rgba(255,255,255,0.1)'; e.target.style.boxShadow = 'inset 0 2px 4px rgba(0,0,0,0.3)'; }}
                />
              </div>
            </div>
