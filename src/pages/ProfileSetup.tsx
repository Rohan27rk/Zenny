...
import { User, Phone, Briefcase, Target, ChevronRight, Check } from 'lucide-react';
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden"
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                ...
                <div className="absolute bottom-0 right-1/3 w-96 h-96 rounded-full opacity-8"
            ...
            <div className="w-full max-w-md relative z-10">
                ...
                <div className="text-center mb-6">
                    ...
                    <p className="text-slate-500 text-sm mt-1">Takes about 2 minutes · You can edit this later</p>
                </div>
                    ...
                    {steps.map((s, i) => {
                        ...
                        return (
                            <div key={i} className="flex items-center gap-2">
                                ...
                                {i < steps.length - 1 && <div className="w-4 h-px" style={{ background: 'rgba(255,255,255,0.1)' }} />}
                            </div>
                        );
                    })}
                </div>

                <div className="rounded-3xl border border-white/10 p-7 backdrop-blur-xl"
                    ...
                    {step === 1 && (
                        <div className="space-y-4">
                            <div>
                                ...
                                <input className={inputBase} style={inputStyle} placeholder="Rohan Kanegaonkar"
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    <div className="px-3 py-3 rounded-xl border border-white/10 text-sm text-slate-400 flex-shrink-0"
                                        style={{ background: 'rgba(0,0,0,0.35)' }}>+91</div>
                                    <input className={inputBase} style={inputStyle} placeholder="9876543210" type="tel"
                            ...
                            <button onClick={() => { if (!form.full_name.trim()) { setError('Please enter your name'); return; } setError(''); setStep(2); }}
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <select className={inputBase} style={{ ...inputStyle, appearance: 'none' }}
                                    ...
                                    onFocus={onFocus} onBlur={onBlur}>
                                    <option value="" style={{ background: '#1e1e30' }}>Select occupation</option>
                                    {OCCUPATIONS.map(o => <option key={o} value={o} style={{ background: '#1e1e30' }}>{o}</option>)}
                                </select>
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-1.5">Monthly Income (approx.)</label>
                                ...
                                <p className="text-xs text-slate-600 mt-1">Used to calculate your savings rate. Not shared anywhere.</p>
                            </div>
                        ...
                        <form onSubmit={handleFinish} className="space-y-5">
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-3">
                                    Monthly Savings Goal — <span className="text-blue-400 font-bold">{form.savings_goal_pct}%</span>
                                </label>
                                ...
                                {form.monthly_income && (
                                    <div className="mt-3 p-3 rounded-xl border border-white/8 text-center"
                                        style={{ background: 'rgba(59,130,246,0.08)' }}>
                                        <p className="text-xs text-slate-400">Monthly savings target</p>
                                        <p className="text-lg font-bold text-blue-400 mt-0.5">
                                            ₹{Math.round(Number(form.monthly_income) * form.savings_goal_pct / 100).toLocaleString('en-IN')}
                                        </p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="p-4 rounded-xl border border-white/8" style={{ background: 'rgba(255,255,255,0.03)' }}>
                                <p className="text-xs font-semibold text-slate-400 mb-2">Your profile summary</p>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button type="submit" disabled={loading}
                                    ...
                                    {loading ? 'Saving...' : <><Check className="w-4 h-4" /> Let's go!</>}
                                </button>
                            </div>
                        </form>
                    )}
